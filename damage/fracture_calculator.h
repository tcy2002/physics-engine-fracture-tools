...
#include "damage/voronoi_calculator.h"
#include "common/hash_vector.h"
#include <algorithm>
...
private:
    ...
    void cut_mesh_of_one(const triangle_manager& mesh, uint32_t idx, triangle_manager& new_mesh) {
        ...
        std::cout << adjacent_point_ids.size() << std::endl;
...
public:
    void triangulate(const std::vector<Vector3>& sample_points) {
        _voronoi.triangulate(sample_points);
    }

    void fracture(const simple_mesh& mesh, std::vector<simple_mesh>& result) {
