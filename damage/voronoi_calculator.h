...
#include "damage/triangle_manager.h"
...
private:
    ...
    void add_bounding_box(const std::vector<Vector3>& points) {
        // find aabb bounds
        Vector3 min(REAL_MAX, REAL_MAX, REAL_MAX);
        Vector3 max(REAL_MIN, REAL_MIN, REAL_MIN);
        for (auto& p : points) {
    ...
    void generate(const std::vector<Vector3>& points) {
        ...
        for (auto& point : points) {
            ...
            for (uint32_t i = good_triangles.size() - 1; i != NOT_FOUND; i--) {
                ...
                if (is_point_on_plane(point, v1, (v2 - v1).cross(v3 - v1))) {
