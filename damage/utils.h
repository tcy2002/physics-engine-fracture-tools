...
FORCE_INLINE void calc_tetrahedron_bounding_sphere(const Vector3& v1, const Vector3& v2, const Vector3& v3, const Vector3& v4, Vector3& center, Real& radius) {
    ...
    auto v1v2v3n = v1v2.cross(v1v3), v1v3v4n = v1v3.cross(v1v4);

    auto v1v2n = v1v2.cross(v1v2v3n);
    Real k1 = v1v3.dot(v1v3m - v1v2m) / v1v2n.dot(v1v3);
    auto p1 = v1v2m + v1v2n * k1;

    auto v1v3n = v1v3.cross(v1v3v4n);
    Real k2 = v1v4.dot(v1v4m - v1v3m) / v1v3n.dot(v1v4);
    auto p2 = v1v3m + v1v3n * k2;

    Real k3 = v1v4.dot(p2 - p1) / v1v2v3n.dot(v1v4);
...
FORCE_INLINE bool are_points_collinear(const Vector3& v1, const Vector3& v2, const Vector3& v3) {
    return approx_equal((v1 - v2).cross(v3 - v2).norm(), 0);
}
...
uint32_t hash_func(const physeng::Vector3& v) {
    auto x = (uint32_t)round(v.x() / EPS);
    auto y = (uint32_t)round(v.y() / EPS);
    auto z = (uint32_t)round(v.z() / EPS);
...
bool equal(const physeng::Vector3& a, const physeng::Vector3& b) {
    return physeng::approx_equal(a, b);
}

bool equal(const physeng::vertex& a, const physeng::vertex& b) {
    return physeng::approx_equal(a.pos, b.pos) && physeng::approx_equal(a.nor, b.nor);
}
...
bool equal(const physeng::polygon& a, const physeng::polygon& b) {
    return physeng::approx_equal(a.nor, b.nor);
}
